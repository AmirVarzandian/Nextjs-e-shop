...
const SelectedColor:React.FC<SelectedColorProps> = ({item , addImageToState , removeImageFromState , isProductCreated}) => {
    ...
    return (
        <div className="grid grid-cols-1 overflow-y-auto border-b-[1.2px] border-slate-200  items-center p-2">
            ...
            {file && (
                <div className="flex flex-row gap2 text-sm col-span-2 items-center justify-between">
                        <p>{file?.name}</p>
                        <div className="w-70px">
                                <CustomeBtn label="Cancel" small outline onClick={()=>{
                                    setFile(null)
                                    removeImageFromState(item)
                                }}/>
                        </div>
